...
export default function Home() {
  ...
  const [result, setResult] = useState<{ putts: 1 | 2 | 3 | 4; makePctUsed?: number } | null>(null);
  ...
  const handleCalculate = useCallback(async (distanceFeet: number) => {
    setLoading(true);
    setResult(null);
    try {
      ...
      <header className="flex flex-wrap items-center justify-between gap-2 mb-6">
        ...
        <nav className="flex items-center gap-3">
          {status === "loading" ? (
            ...
            <>
              <Link href="/signin" className="text-sm underline text-neutral-600 hover:text-neutral-900">
                Sign in
              </Link>
              <Link href="/signup" className="text-sm underline text-neutral-600 hover:text-neutral-900">
                Sign up
              </Link>
            </>
          )}
