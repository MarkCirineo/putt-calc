...
/** Make-% bands (ft): 0–3, 3–6, 6–9, 9–12, 12–18, 18–24, 24–30, 30+ */
export const MAKE_PCT_BANDS = [
  ...
  { minFt: 30, maxFt: 999, label: "30+ ft" },
] as const;

/** Make-% by band index × HCP (0,5,10,15,20,25). Source: plan table. */
export const MAKE_PCT_TABLE: Record<HandicapIndex, number[]> = {
  0: [98, 76, 49, 34, 19, 12, 7, 4],
  5: [96, 67, 44, 34, 19, 13, 7, 3],
  10: [96, 65, 39, 26, 18, 10, 7, 3],
  15: [93, 59, 36, 22, 16, 9, 7, 2],
  20: [90, 55, 33, 18, 14, 7, 5, 2],
  25: [88, 48, 30, 17, 12, 6, 4, 2],
};

/** 3-putt % bands (ft): 10, 11–15, 16–20, 21–25, 26–30, 31–35, 36–40 */
export const THREE_PUTT_BANDS = [
  ...
  { minFt: 41, maxFt: 999, label: "40+ ft" },
] as const;

/** 3-putt % by band index × HCP (0,5,10,15,20). Example values from plan; tune as needed. */
export const THREE_PUTT_TABLE: Record<HandicapIndex, number[]> = {
  0: [1.7, 4, 8, 12, 16, 20, 25.3, 30],
  5: [2.5, 5, 10, 14, 18, 22, 28, 35],
  10: [3, 6, 12, 17, 22, 27, 33, 40],
  15: [4, 8, 14, 19, 25, 30, 37, 44],
  20: [8.4, 14, 20, 27, 33, 40, 48.3, 55],
  25: [10, 18, 26, 34, 42, 50, 58, 65],
};
...
export function getMakePctBandIndex(distanceFeet: number): number {
  for (let i = 0; i < MAKE_PCT_BANDS.length; i++) {
    ...
    if (distanceFeet >= b.minFt && (b.maxFt > 999 || distanceFeet < b.maxFt))
      return i;
  }
